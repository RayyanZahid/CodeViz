 ...
 */
function wireInferenceBroadcast(engine) {
    engine.on('inference', (result) => {
...
inferenceEngine.on('inference', (result) => {
    const parts = [];
    if (result.zoneUpdates.length > 0)
        parts.push(`${result.zoneUpdates.length} zone updates`);
    if (result.architecturalEvents.length > 0)
        parts.push(`${result.architecturalEvents.length} arch events`);
    if (result.risks.length > 0)
        parts.push(`${result.risks.length} risks`);
...
let pipeline = new Pipeline(currentWatchRoot, (batch) => graph.onParseResult(batch));
// ---------------------------------------------------------------------------
 ...
 */
async function switchWatchRoot(newDir) {
