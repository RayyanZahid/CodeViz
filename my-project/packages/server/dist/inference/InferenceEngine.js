...
export class InferenceEngine extends EventEmitter {
    ...
    loadPersistedZones() {
        ...
        for (const row of nodes) {
            if (row.zone !== null && row.zone !== undefined) {
                this.classifier.updateZoneCache(row.id, row.zone);
            }
        }
    }
    /**
    ...
    processDelta(delta) {
        ...
        const risks = this.riskDetector.detectRisks(delta, (id) => this.getZoneForNode(id), (id) => this.graph.getOutEdges(id));
