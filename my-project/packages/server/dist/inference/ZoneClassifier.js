 ...
 */
export const ZONE_PATH_PATTERNS = [
    {
        zone: 'frontend',
        patterns: [
...
export class ZoneClassifier {
    ...
    zoneCache = new Map();
    constructor(configLoader, graph) {
    ...
    classifyDelta(delta) {
        ...
        const pass1Classified = new Set(); // tracks nodes that changed to non-unknown in Pass 1
        ...
        if (pass1Classified.size > 0) {
            for (const [nodeId, cachedZone] of this.zoneCache) {
                if (cachedZone !== 'unknown')
                    continue; // only re-evaluate unknowns
     ...
     */
    classifyByTopology(nodeId) {
        const predecessors = this.graph.getPredecessors(nodeId);
        const successors = this.graph.getSuccessors(nodeId);
        // Combine neighbors, filter out external stubs.
        const neighbors = [...predecessors, ...successors].filter((n) => !n.startsWith('__ext__/'));
        ...
        for (const neighbor of neighbors) {
            ...
            if (neighborZone === undefined || neighborZone === 'unknown')
                continue;
...
//# sourceMappingURL=ZoneClassifier.js.map