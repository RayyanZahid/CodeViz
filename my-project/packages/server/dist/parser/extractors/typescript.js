 ...
 */
export function extractTypeScript(rootNode, _filePath) {
...
function extractExports(rootNode) {
    ...
    for (const node of rootNode.namedChildren) {
        ...
        if (exportClause) {
            for (const item of exportClause.namedChildren) {
                ...
                if (item.type === 'export_specifier') {
                    ...
                    const names = item.children.filter((c) => c.type === 'identifier' || c.type === 'property_identifier');
        ...
        const declaration = node.children.find((c) => c.type === 'function_declaration' ||
            c.type === 'class_declaration' ||
            c.type === 'lexical_declaration' ||
            c.type === 'variable_declaration');
        if (declaration) {
            const nameNode = declaration.children.find((c) => c.type === 'identifier' || c.type === 'variable_declarator');
            if (nameNode) {
...
function extractCalls(rootNode) {
    const calls = [];
    function walk(node, depth, insideFunction) {
        ...
        const entersFunctionScope = node.type === 'function_declaration' ||
            node.type === 'function' ||
