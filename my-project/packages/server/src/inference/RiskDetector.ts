 ...
 */
export const ZONE_LAYER_ORDER: Partial<Record<ZoneName, number>> = {
  frontend: 0,
  api: 1,
  services: 2,
  'data-stores': 3,
...
export class RiskDetector {
  ...
  private detectFanOutRisks(
    ...
    for (const nodeId of candidates) {
      ...
      const internalOutDegree = allOutEdges.filter(
        (e) => !e.w.startsWith('__ext__/'),
      ).length;
      ...
      if (internalOutDegree > FAN_OUT_THRESHOLD) {
        risks.push({
          ...
          nodeId,
          details:
            `Excessive fan-out: ${nodeId} has ${internalOutDegree} internal ` +
